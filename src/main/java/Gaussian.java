...
public class Gaussian {
  ...
  public static void main(String[] args) throws IOException {
    ...
    /* Reading input image and creating output image */
    BufferedImage input = ImageIO.read(new File("images/input.jpg"));
    BufferedImage output = new BufferedImage(input.getWidth(), input.getHeight(), BufferedImage.TYPE_INT_RGB);

    /* Setting kernel size and standard deviation for Gaussian blur */
    int kernelSize = 5;
    double sigma = 1.5;

    /* Creating and initializing the Gaussian kernel */
    ...
    for (int i = 0; i < input.getWidth(); i++) {
        for (int j = 0; j < input.getHeight(); j++) {
            ...
            for (int ki = 0; ki < kernelSize; ki++) {
                for (int kj = 0; kj < kernelSize; kj++) {
                    ...
                    if (pi >= 0 && pi < input.getWidth() && pj >= 0 && pj < input.getHeight()) {
                        ...
                        /* Applying the kernel to the color values */
            ...
            /* Rounding color values to integers and clamping them to 0-255 range */
